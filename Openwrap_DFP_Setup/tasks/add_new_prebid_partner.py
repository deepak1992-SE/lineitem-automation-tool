...
from Openwrap_DFP_Setup.dfp import associate_line_items_and_creatives
from Openwrap_DFP_Setup.dfp import create_custom_targeting
from Openwrap_DFP_Setup.dfp import create_creatives
from Openwrap_DFP_Setup.dfp import create_line_items
from Openwrap_DFP_Setup.dfp import create_orders
from Openwrap_DFP_Setup.dfp import get_ad_units
from Openwrap_DFP_Setup.dfp import get_advertisers
from Openwrap_DFP_Setup.dfp import get_custom_targeting
from Openwrap_DFP_Setup.dfp import get_placements
from Openwrap_DFP_Setup.dfp import get_users
from Openwrap_DFP_Setup.dfp import get_root_ad_unit_id
...
def setup_partner(user_email, advertiser_name, order_name, placements, ad_units, sizes, bidder_code, prices,
  ...
  # Get the user.
  user_id = get_users.get_user_id_by_email(user_email)
  ...
  ad_unit_ids=None
  if len(placements)>0:
    placement_ids = get_placements.get_placement_ids_by_name(placements)

  # Get the ad unit IDs.
  if len(ad_units)>0:
    ad_unit_ids = get_ad_units.get_ad_unit_ids_by_name(ad_units)
  # Get (or potentially create) the advertiser.
  else:
    root_id = get_root_ad_unit_id.get_root_ad_unit_id()
    ad_unit_ids = [ root_id ]

  
  advertiser_id = get_advertisers.get_advertiser_id_by_name(
    advertiser_name)

  # Create the order.
  order_id = create_orders.create_order(order_name, advertiser_id, user_id)

  # Create creatives.
  creative_configs = create_creatives.create_duplicate_creative_configs(
      bidder_code, order_name, advertiser_id, num_creatives=num_creatives)
  creative_ids = create_creatives.create_creatives(creative_configs)
  ...
  line_item_ids = create_line_items.create_line_items(line_items_config)

  # Associate creatives with line items.
  associate_line_items_and_creatives.make_licas(line_item_ids,
...
def create_line_item_configs(prices, order_id, placement_ids, ad_unit_ids, bidder_code,
  ...
  for price in prices:
    ...
    config = create_line_items.create_line_item_config(
      name=line_item_name,
      order_id=order_id,
